...
def upgrade() -> None:
    ...
    op.create_table('games',
        ...
        sa.Column('player2_type', sa.Enum('human', 'ai', name='player2type'), nullable=True),
        sa.Column('board_state', sa.JSON(), nullable=True),
        sa.Column('current_turn', sa.Enum('X', 'O', name='turn'), nullable=True),
        sa.Column('status', sa.Enum('waiting', 'in_progress', 'completed', 'abandoned', name='gamestatus'), nullable=True),
    ...
    op.create_table('game_observers',
        sa.Column('id', sa.Integer(), nullable=False),
