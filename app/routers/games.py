...
from app.models.game import GameCreate, GameResponse, GameMove, GameListItem
...
async def join_game(
    ...
    websocket_manager = get_websocket_manager()
    await websocket_manager.notify_game_update(game_id, GameResponse.from_orm(game).dict())
    await websocket_manager.notify_games_list_update()
...
async def observe_game(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot observe this game"
        )
